...
const fetchStars = async () => {
  if (!cache.data || cache?.time < Date.now()) {
    const fe = await axios.get(
      "https://adventofcode.com/2021/leaderboard/private/view/641193.json",
      {
        headers: {
    ...
    const json = fe.data;
    const stars = Object.values(json.members).reduce(
      (sum, { stars }) => sum + stars,
      0
    );
    cache.data = json;
    cache.stars = stars;
    cache.time = Date.now() + 90000;
  }
};
...
const sendGoodMorning = async () => {
  ...
  if (cache?.stars > 3000) {
    ...
    text += `that is about ${
      Math.round((cache?.stars / 3000) * 10000) / 100
    } percent so far`;
  } else {
    text += `⭐ behind of goal: ${3000 - cache?.stars}, let's gooooo! 🙌🥳`;
  }
  text += `\nTodays challenge can be found here: https://adventofcode.com/2021/day/${new Date(
    Date.now()
  ).getDate()}`;
